## Update notes ##
# Heuristic algorthim has been added. 14/03/22
...
class DQN(nn.Module):
    ...
    def __init__(self, learning_rate=0.01, state_size=47, 
                 action_size=11, hidden_size=64, hidden_size_1=64, hidden_size_2=32, batch_size=20, gamma=0.90):
...
def Heuristic_select_action(state):
    ...
    action = np.random.choice(destinations,size=1) # Choose random action from destinations
    ...
    if mmWave_available == 1: 
        action = action*2+1
    elif mmWave_available == 0 and LTE_available == 1:
        action = action *2 
    else:
        action = 11
...
if __name__ == "__main__":
    ...
    # for ii in range(0, len(random_seeds)):
        ...
        for b in range(num_eps):
            ...
            print('Training episode: {} in time {}'.format(b,elapsed_t))
            reward_list = []
            obs = env.reset()
        ...
        print('Finished jobs: ', Heuristic_finished_jobs/num_eps/n_cpu)
        print('Lost jobs (deadline): ', Heuristic_exceed_deadlines/num_eps/n_cpu)
        print('Average caching rate: ', Heuristic_finished_jobs/(Heuristic_finished_jobs+Heuristic_exceed_deadlines))
        print('Finished job size: ', Heuristic_finished_job_size/num_eps/n_cpu*1)
        print('Lost job size(deadline)', Heuristic_exceed_deadline_job_size/num_eps/n_cpu*1)
        ...
        print('Finished jobs: ', Training_finished_jobs/num_eps/n_cpu)
        print('Lost jobs (deadline): ', Training_exceed_deadlines/num_eps/n_cpu)
        print('Average caching rate: ', Training_finished_jobs/(Training_finished_jobs+Training_exceed_deadlines))
        print('Finished job size: ', Training_finished_job_size/num_eps/n_cpu*1)
        print('Lost job size(deadline)', Training_exceed_deadline_job_size/num_eps/n_cpu*1)
        ...
        plt.plot(B[0:12], label='Training')
