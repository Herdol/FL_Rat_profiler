...
if __name__ == "__main__":
    ...
    for ii in range(1): # This loop will be used for FL UEs
        ...
        for b in range(num_eps):
            ...
            metrics = {'Heuristic_finished_jobs': job_history['completed_jobs'],
                    'Heuristic_finished_job_size': np.sum(job_history['completed_jobs_size']),
                    'Heuristic_exceed_deadlines': job_history['exceed_deadline'],
                    'Heuristic_exceed_deadline_job_size': np.sum(job_history['exceed_deadline_size']),
                    'Heuristic Average caching rate: ': Heuristic_finished_jobs/(Heuristic_finished_jobs+Heuristic_exceed_deadlines),
        ...
        axs2[0, 0].plot(Training_finished_jobs_log[0:num_eps], label='Training')
        axs2[0, 0].plot(finished_jobs_log, label='Validation')
        axs2[0, 0].plot(Heuristic_finished_jobs_log, label='Heuristic')
        axs2[0, 0].legend(loc="upper left")
        axs2[0, 0].set_title('finished_jobs')
        axs2[0, 0].grid()
        axs2[0, 1].plot(Training_finished_job_size_log[0:num_eps], label='Training')
        axs2[0, 1].plot(finished_job_size_log, label='Validation')
        axs2[0, 1].plot(Heuristic_finished_job_size_log, label='Heuristic')
        axs2[0, 1].legend(loc="upper left")
        axs2[0, 1].set_title('finished_job_size')
        axs2[0, 1].grid()
        axs2[1, 0].plot(Training_exceed_deadlines_log[0:num_eps], label='Training')
        axs2[1, 0].plot(exceed_deadlines_log, label='Validation')
        axs2[1, 0].plot(Heuristic_exceed_deadlines_log, label='Heuristic')
        axs2[1, 0].legend(loc="upper left")
        axs2[1, 0].set_title('exceed_deadlines')
        axs2[1, 0].grid()
        axs2[1, 1].plot(Training_exceed_deadline_job_size_log[0:num_eps], label='Training')
        axs2[1, 1].plot(exceed_deadline_job_size_log, label='Validation')
        axs2[1, 1].plot(Heuristic_exceed_deadline_job_size_log, label='Heuristic')
        axs2[1, 1].legend(loc="upper left")
        axs2[1, 1].set_title('exceed_deadline_job_size')
        axs2[1, 1].grid()
        
        A=np.array(Heuristic_finished_jobs_log)/(np.array(Heuristic_finished_jobs_log)+np.array(Heuristic_exceed_deadlines_log))
        B=np.array(Training_finished_jobs_log)/(np.array(Training_finished_jobs_log)+np.array(Training_exceed_deadlines_log))
        C=np.array(finished_jobs_log)/(np.array(finished_jobs_log)+np.array(exceed_deadlines_log))
        plt.plot(B, label='Training')
        plt.plot(C, label='Validation')
        plt.plot(A, label='Heuristic')
        plt.legend(loc="upper left")
        plt.title("Average caching rate")
